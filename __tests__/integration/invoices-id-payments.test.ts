...
function req(method='GET', body?:unknown, id: string='test-id'): [NextRequest, { params: Promise<{id:string}> }] {
  ...
  return [new NextRequest(new URL('http://localhost:3008/api/invoices/[id]/payments'), init), { params: Promise.resolve({ id }) }];
}
