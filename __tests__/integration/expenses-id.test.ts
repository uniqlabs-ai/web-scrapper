...
function req(method='GET', body?:unknown, id: string='test-id'): [NextRequest, { params: Promise<{id:string}> }] {
  ...
  return [new NextRequest(new URL('http://localhost:3008/api/expenses/[id]'), init), { params: Promise.resolve({ id }) }];
}
