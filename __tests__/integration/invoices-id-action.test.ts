...
function req(method='GET', body?:unknown, id: string='test-id', action: string='test-id'): [NextRequest, { params: Promise<{id:string; action:string}> }] {
  const init: Record<string,unknown> = { method };
  if (body) { init.body=JSON.stringify(body); init.headers={'Content-Type':'application/json'}; }
  return [new NextRequest(new URL('http://localhost:3008/api/invoices/[id]/[action]'), init), { params: Promise.resolve({ id, action }) }];
}
